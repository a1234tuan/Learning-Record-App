...
复习效果驱动的 AI 助教正在 `feature/review-effect-coach-v2` 分支开发，产品与实施边界见 [docs/新的方案.md](docs/新的方案.md)。截至 2026-09-07，阶段 0-6 已完成：schema 17 数据底座、决策块纵向生命周期、记录级复习接入、块级 AI 上下文、正式数据的备份/同步支持、块级评论与待分析队列、快速模型异步评论整理、首页深度分析工作台与任务调度，以及独立自适应训练页均已落地。阶段 6 支持 Blueprint 约束下的逐轮出题、客观/计算/唯一答案题独立质检、提示记录、作答评估、跳过与题目无效处置、主观收口和未掌握重新入队；作答与正式结果原子保存，作答前不显示答案依据。阶段 7 的延迟验证与效果学习尚未开始。
