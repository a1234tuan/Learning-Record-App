...
- **间隔复习**：把学习记录加入复习队列，按“记住了 / 模糊 / 忘记了”更新下一次复习日期。
...
- 到期卡片出现在复习队列。
- 评分按钮为：记住了、模糊、忘记了。
- 当天已评分的卡片会立即离开今日队列，避免旧数据刷新后重复出现。
- 已掌握或已搁置的卡片不会继续进入到期队列。
- 卡片管理中可搜索、按学科筛选、查看状态、重置或搁置。

当前模型是简化间隔重复模型，目标是保证关键体验不变量：答过即离开当天队列、重复点击不重复写入、搁置/掌握不再到期。它不是完整 Anki 状态机，也没有 Again/Hard/Good/Easy 四按钮。
...
Android 端支持原生流式备份，适合生成较大的 zip。备份包中的 `entries/*.md` 也会包含记录正文，方便不恢复数据时快速预览内容。
...
- 修改复习逻辑时，需要重点验证“当天已评分不再出现”“重复点击不重复写入”“搁置/掌握不进入队列”等不变量。
