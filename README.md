...
复习效果驱动的 AI 助教正在 `feature/review-effect-coach-v2` 分支开发，产品与实施边界见 [docs/新的方案.md](docs/新的方案.md)。截至 2026-09-06，阶段 0-3 已完成：schema 17 数据底座、决策块纵向生命周期、记录级复习接入、块级 AI 上下文、正式数据的备份/同步支持，以及块级评论与待分析队列均已落地。阶段 3 仍未接入 AI 调用，下一步才进入阶段 4 的快速模型评论整理。
