...
复习效果驱动的 AI 助教正在 `feature/review-effect-coach-v2` 分支开发，产品与实施边界见 [docs/新的方案.md](docs/新的方案.md)。截至 2026-09-07，阶段 0-5 已完成：schema 17 数据底座、决策块纵向生命周期、记录级复习接入、块级 AI 上下文、正式数据的备份/同步支持、块级评论与待分析队列、快速模型异步评论整理，以及首页深度分析工作台与任务调度均已落地。阶段 5 支持手动确认、最多三块并受 token 上限约束的自动拆批、严格来源与版本校验、`SessionBlueprint`、部分失败恢复，以及全局唯一当前任务、等待、延期和确认切换。阶段 6 的自适应测验生成与执行尚未开始。
