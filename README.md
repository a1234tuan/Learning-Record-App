...
复习效果驱动的 AI 助教正在 `feature/review-effect-coach-v2` 分支开发，产品与实施边界见 [docs/新的方案.md](docs/新的方案.md)。截至 2026-09-07，阶段 0-7 已完成：schema 18 数据底座、决策块纵向生命周期、记录级复习接入、块级 AI 上下文、正式数据的备份/同步支持、块级评论与待分析队列、快速模型异步评论整理、首页深度分析工作台与任务调度、独立自适应训练页，以及延迟验证与效果学习均已落地。即时训练完成后由本地策略安排验证窗口；验证复用已接受 Blueprint，但必须生成不同于历史题目的新题，保持/衰退结果与整条日志的 FSRS 分开保存。效果摘要按问题类型、练习类型、Provider、模型、Prompt 和策略版本隔离，小样本只显示“证据不足”。下一阶段为阶段 8 的完整迁移、同步与旧入口替换。
